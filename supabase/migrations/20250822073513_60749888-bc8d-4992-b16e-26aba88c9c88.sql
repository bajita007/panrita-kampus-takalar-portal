...
BEGIN
  -- Insert admin user into auth.users (this is for reference, actual user creation happens via Supabase Auth)
  -- We'll create profiles for default users that can be used when they sign up
  
  -- Insert default admin profile (user will need to sign up with this email)
  INSERT INTO public.profiles (user_id, email, full_name, role)
  VALUES (gen_random_uuid(), 'admin@itptakalar.ac.id', 'Admin ITP', 'admin');
  
  -- Insert default super admin profile
  INSERT INTO public.profiles (user_id, email, full_name, role)
  VALUES (gen_random_uuid(), 'superadmin@itptakalar.ac.id', 'Super Admin ITP', 'super_admin');
END $$;
